...
* Interactive API documentation (Scramble + Scalar)
...
and then exchanges the credentials at `POST /api/v1/oauth/token`.
...
The TaskFlow API is versioned with the `v1` path prefix — every endpoint, including the Passport OAuth2 server and its helper endpoints, lives under `/api/v1/*`.
...
The backend ships with interactive API documentation generated by Scramble and rendered with Scalar.
...
The raw OpenAPI 3.1 specification is served at:

```text
GET /api/docs.openapi.json
```

The specification describes the full API:

* every endpoint is versioned under `v1` — the server is `http://localhost:8080` and operation paths keep the full `/api/v1` prefix (e.g. `POST /api/v1/register`, `POST /api/v1/oauth/token`)
* endpoints grouped into Authentication, OAuth2, Tasks, Projects, and Comments
* both authentication security schemes

Authenticated endpoints require two security schemes:

* `Authorization: Bearer <token>` — HTTP bearer token (Sanctum, JWT, or Passport)
* `X-Auth-Method: sanctum|jwt|passport` — header selecting the authentication strategy; documented as a companion header, not an API key

The Passport token flow (`POST /api/v1/oauth/token` and `/api/v1/oauth/token/refresh`) is grouped under OAuth2. The unused authorization-code and device flows are excluded from the generated specification.

Endpoint sections are driven by controller PHPDoc summaries and the `#[Group]` attribute.
